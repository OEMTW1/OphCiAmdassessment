...
/**
 * This is the model class for table "et_ophciamdassessment_visualacuity".
 *
 ...
 * @property OphCiAmdassessment_VisualAcuityUnit $unit
 * @property OphCiAmdassessment_VisualAcuity_Reading[] $readings
 * @property OphCiAmdassessment_VisualAcuity_Reading[] $left_readings
 * @property OphCiAmdassessment_VisualAcuity_Reading[] $right_readings
 * @property User $user
...
class Element_OphCiAmdassessment_VisualAcuity extends SplitEventTypeElement
{
	public $service;
	...
	{
		return 'et_ophciamdassessment_visualacuity';
	}
	...
	public function relations()
	{
		...
		return array(
				...
				'unit' => array(self::BELONGS_TO, 'OphCiAmdassessment_VisualAcuityUnit', 'unit_id'),
				'uniteee' => array(self::BELONGS_TO, 'OphCiAmdassessment_VisualAcuityUnit', 'unit_id'),
				'readings' => array(self::HAS_MANY, 'OphCiAmdassessment_VisualAcuity_Reading', 'element_id'),
				'right_readings' => array(self::HAS_MANY, 'OphCiAmdassessment_VisualAcuity_Reading', 'element_id', 'on' => 'right_readings.side = 0'),
				'left_readings' => array(self::HAS_MANY, 'OphCiAmdassessment_VisualAcuity_Reading', 'element_id', 'on' => 'left_readings.side = 1'),
		);
	}
	...
	public function getUnit()
	{
		...
		return OphCiAmdassessment_VisualAcuityUnit::model()->findByPk($unit_id);
	}
	 ...
	 *
	 * @return OphCiAmdassessment_VisualAcuityUnit[]
	 */
	public function getUsableUnits()
	{
		...
		$criteria->order = 'name';
		return OphCiAmdassessment_VisualAcuityUnit::model()->findAll($criteria);
	}
	...
	{
		if ($unit_id) {
			$unit = OphCiAmdassessment_VisualAcuityUnit::model()->findByPk($unit_id);
		} else {
			$unit = $this->unit;
		}
		if ($selectable) {
	...
	public function getUnitValuesForForm($unit_id = null)
	{
		...
		$criteria->order = 'name';
		$tooltip_units = OphCiAmdassessment_VisualAcuityUnit::model()->findAll($criteria);
		...
		if (is_array($readings)) {
			foreach ($readings as $reading) {
				if ($reading['side'] == $side_id) {
					$reading_model = new OphCiAmdassessment_VisualAcuity_Reading();
		...
		if (isset($_POST['visualacuity_readings_valid']) && $_POST['visualacuity_readings_valid']) {
			...
			foreach ($new_readings as $reading) {
				...
				if (isset($reading['id']) && isset($existing_reading_ids[$reading['id']])) {
					...
					$reading_model = OphCiAmdassessment_VisualAcuity_Reading::model()->findByPk($reading['id']);
			...
			OphCiAmdassessment_VisualAcuity_Reading::model()->deleteByPk(array_values($existing_reading_ids));
	...
	{
		$unit_id = null;
		if ($unit = OphCiAmdassessment_VisualAcuityUnit::model()->find('name = ?', array('Snellen Metre'))) {
			$unit_id = $unit->id;
		}
